...
public:
    ...
    vector<double> class_vector; // for deep forest
};
...
void create_leaf(Task_Master* task_master) {
    ...
    if(task_master->task_type == TASK_COL_SPLIT) {
        ...
        size = task->size;
        class_vector.swap(task->class_vector);
    ...
    if(data_type == ELEM_BOOL) {
        ...
        leafNode->class_vector.swap(class_vector);
...
#ifdef ASSERT
    assert(task_master->node->class_vector.size() == y_classes.size());
#endif
}

void set_node_label(Matrix & dataset, vector<size_t> & rows, TreeConfig & treeConfig,
                    string & node_label, vector<double> & class_vector) {
    ...
    if(Y->data_type == ELEM_BOOL) { // only categorical
        ...
        if(!Y->is_ordinal) {
            stop_splitting_categorical<bool>(cserver.X, start, end, treeConfig, label, class_vector);
        } else {
        ...
        if(Y->is_ordinal) {
            ...
            stop_splitting_categorical<short>(cserver.X, start, end, treeConfig, label, class_vector);
        }
        ...
        if(Y->is_ordinal) {
            ...
            stop_splitting_categorical<int>(cserver.X, start, end, treeConfig, label, class_vector);
        }
        ...
        if(Y->is_ordinal) {
            ...
            stop_splitting_categorical<float>(cserver.X, start, end, treeConfig, label, class_vector);
        }
        ...
        if(Y->is_ordinal) {
            ...
            stop_splitting_categorical<double>(cserver.X, start, end, treeConfig, label, class_vector);
        }
        ...
        if(!Y->is_ordinal) {
            stop_splitting_categorical<char>(cserver.X, start, end, treeConfig, label, class_vector);
        } else {
            cout << "Error: Wrong type, File = " << __FILE__ << ", Line = " << __LINE__ << endl;
            exit(-1);
        }
        ...
        if(!Y->is_ordinal) {
            stop_splitting_categorical<string>(cserver.X, start, end, treeConfig, label, class_vector);
        } else {
            cout << "Error: Wrong type, File = " << __FILE__ << ", Line = " << __LINE__ << endl;
            exit(-1);
        }
