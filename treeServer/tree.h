...
bool stop_splitting_categorical(Matrix & dataset, vector<size_t>::iterator start, vector<size_t>::iterator end,
                    TreeConfig &treeConfig, T &leaf_label, vector<double> & class_vector) { // checking whether all Y_values are same

    Column* Y = dataset.get_column(y_index);
    class_vector.resize(y_classes.size());
    ...
    for(auto it = freq.begin(); it != freq.end(); it++) {
        ...
        class_vector[ymap[key]] = (double) it->second / D;
    }
...
template <class T>
TreeNode* create_leaf(vector<size_t>::iterator start, vector<size_t>::iterator end,
                      T label, vector<double> & class_vector)
{
...
template <class T>
void set_categorical_node(TreeNode* &node, SplitResult *result, string node_label, vector<double> & class_vector) {
    ...
    treeNode->class_vector.swap(class_vector);
}
...
TreeNode* build_tree(Task_Slave_Subtree* task, vector<size_t>::iterator start, vector<size_t>::iterator end, int tree_depth) {
    ...
    if(Y->is_ordinal) {
        if(ytype == ELEM_SHORT) {
            ...
            if(stop || end_of_path) {
                TreeNode* node = create_leaf<short>(start, end, label, class_vector);
                return node;
            }
            ...
            if(stop || end_of_path) {
                TreeNode* node = create_leaf<int>(start, end, label, class_vector);
                return node;
            }
            node_label = to_string(label);
            ...
            if(stop || end_of_path) {
                TreeNode* node = create_leaf<float>(start, end, label, class_vector);
                return node;
            }
            node_label = to_string(label);
            ...
            if(stop || end_of_path) {
                TreeNode* node = create_leaf<double>(start, end, label, class_vector);
                return node;
            }
            node_label = to_string(label);
        } else {
        ...
        if(ytype == ELEM_BOOL) {
            bool label;
            stop = stop_splitting_categorical<bool>(*(task->matrix), start, end, treeConfig, label, class_vector);
            if(stop || end_of_path) {
                TreeNode* node = create_leaf<bool>(start, end, label, class_vector);
                return node;
            }
            node_label = to_string(label);
        } else if(ytype == ELEM_SHORT) {
            short label;
            stop = stop_splitting_categorical<short>(*(task->matrix), start, end, treeConfig, label, class_vector);
            if(stop || end_of_path) {
                TreeNode* node = create_leaf<short>(start, end, label, class_vector);
                return node;
            }
            ...
            int label;
            stop = stop_splitting_categorical<int>(*(task->matrix), start, end, treeConfig, label, class_vector);
            if(stop || end_of_path) {
                TreeNode* node = create_leaf<int>(start, end, label, class_vector);
                return node;
            }
            ...
            float label;
            stop = stop_splitting_categorical<float>(*(task->matrix), start, end, treeConfig, label, class_vector);
            if(stop || end_of_path) {
                TreeNode* node = create_leaf<float>(start, end, label, class_vector);
                return node;
            }
            ...
            double label;
            stop = stop_splitting_categorical<double>(*(task->matrix), start, end, treeConfig, label, class_vector);
            if(stop || end_of_path) {
                TreeNode* node = create_leaf<double>(start, end, label, class_vector);
                return node;
            }
            ...
            char label;
            stop = stop_splitting_categorical<char>(*(task->matrix), start, end, treeConfig, label, class_vector);
            if(stop || end_of_path) {
                TreeNode* node = create_leaf<char>(start, end, label, class_vector);
                return node;
            }
            node_label = to_string(label);
        } else if(ytype == ELEM_STRING) {
            string label;
            stop = stop_splitting_categorical<string>(*(task->matrix), start, end, treeConfig, label, class_vector);
            if(stop || end_of_path) {
                TreeNode* node = create_leaf<string>(start, end, label, class_vector);
                return node;
            }
            node_label = label;
        } else {
    ...
    if(result->column_idx == -2) {
        TreeNode* leaf_node = create_leaf_wrapper(Y, start, end, node_label, class_vector);
        return leaf_node;
    }
...
#ifdef ASSERT
    assert(class_vector.size() == y_classes.size());
#endif
    ...
    if(Xi->is_ordinal) {
        ...
        if(Xi->data_type == ELEM_BOOL) {
            ...
            ordinalTreeNode->class_vector.swap(class_vector);
        ...
        if(Xi->data_type == ELEM_BOOL) {
            set_categorical_node<bool>(curNode, result, node_label, class_vector);
        } else if(Xi->data_type == ELEM_SHORT) {
            set_categorical_node<short>(curNode, result, node_label, class_vector);
        } else if(Xi->data_type == ELEM_INT) {
            set_categorical_node<int>(curNode, result, node_label, class_vector);
        } else if(Xi->data_type == ELEM_FLOAT) {
            set_categorical_node<float>(curNode, result, node_label, class_vector);
        } else if(Xi->data_type == ELEM_DOUBLE) {
            set_categorical_node<double>(curNode, result, node_label, class_vector);
        } else if(Xi->data_type == ELEM_CHAR) {
            set_categorical_node<char>(curNode, result, node_label, class_vector);
        } else if(Xi->data_type == ELEM_STRING) {
            set_categorical_node<string>(curNode, result, node_label, class_vector);
        } else {
...
void print_categorical_node(TreeNode* root) {
    ...
    for(int i=0; i<treeNode->class_vector.size(); i++)
	{
		cout << treeNode->class_vector[i];
		if(i<treeNode->class_vector.size()-1) cout<<", ";
	}
	cout << "}";
}
...
void print_tree(TreeNode* root, int tree_depth = 0, string indent = "", bool last = true) {
    ...
    if(root->column_index == -1) {
        ...
        if(Y->data_type == ELEM_BOOL) {
            ...
            if(Y->is_ordinal == false)
			{
				cout << ", probs = {";
				for(int i=0; i<leaf_node->class_vector.size(); i++)
				{
					cout << leaf_node->class_vector[i];
					if(i<leaf_node->class_vector.size()-1) cout<<", ";
				}
				cout << "}";
			}
...
template<class T>
void write_categorical_node_hdfs(TreeNode* root, HdfsWriterStream &fout) {
    CategoricalTreeNode<T>* treeNode = (CategoricalTreeNode<T>*) root;
    fout << treeNode->label; // ### write CategoricalTreeNode::label
    fout << treeNode->S; // ### write CategoricalTreeNode::label
    fout << treeNode->S1; // ### write CategoricalTreeNode::label
...
void save_subtree_hdfs(TreeNode* root, HdfsWriterStream & fout)
{
    //for "dataset" we actually just need its meta_data
    //we do not save [start, end)
    ...
    if(root->column_index == -1) {
        ...
        if(Xi->is_ordinal) {
            ...
            if(Xi->data_type == ELEM_BOOL) {
            	write_categorical_node_hdfs<bool>(root, fout);
            } else if(Xi->data_type == ELEM_SHORT) {
            	write_categorical_node_hdfs<short>(root, fout);
            } else if(Xi->data_type == ELEM_INT) {
            	write_categorical_node_hdfs<int>(root, fout);
            } else if(Xi->data_type == ELEM_FLOAT) {
            	write_categorical_node_hdfs<float>(root, fout);
            } else if(Xi->data_type == ELEM_DOUBLE) {
            	write_categorical_node_hdfs<double>(root, fout);
            } else if(Xi->data_type == ELEM_CHAR) {
            	write_categorical_node_hdfs<char>(root, fout);
            } else if(Xi->data_type == ELEM_STRING) {
            	write_categorical_node_hdfs<string>(root, fout);
            } else {
                cout<< "File = " << __FILE__ << ", Line = " << __LINE__ << "[ERROR] Unknown attribute(Xi) of type " << Xi->data_type << endl;
                exit(-1);
            }
        ...
        fout << root->class_vector;
        save_subtree_hdfs(root->left, fout);
        save_subtree_hdfs(root->right, fout);
    }
}

void save_forest_hdfs(const char* path, vector<TreeNode*> forest)
{
	HdfsWriterStream fout(path);
    fout << forest.size();

    for(size_t index = 0; index < forest.size(); index++) {
        save_subtree_hdfs(forest[index], fout);
    }
}
...
template <class T>
void load_categorical_node_hdfs(TreeNode* &result, HdfsReaderStream & fin) {
    CategoricalTreeNode<T>* treeNode = new CategoricalTreeNode<T>;
    fin >> treeNode->label; // ### write CategoricalTreeNode::label
    fin >> treeNode->S; // ### write CategoricalTreeNode::label
    fin >> treeNode->S1; // ### write CategoricalTreeNode::label
...
TreeNode* load_subtree_hdfs(Matrix &dataset, int y_index, HdfsReaderStream & fin)
{
    //for "dataset" we actually just need its meta_data
    //we do not load [start, end)
    int col_idx;
    ...
    if(col_idx == -1) {
        ...
        if(Xi->is_ordinal) {
            ...
            if(Xi->data_type == ELEM_BOOL) {
            	load_categorical_node_hdfs<bool>(result, fin);
            } else if(Xi->data_type == ELEM_SHORT) {
            	load_categorical_node_hdfs<short>(result, fin);
            } else if(Xi->data_type == ELEM_INT) {
            	load_categorical_node_hdfs<int>(result, fin);
            } else if(Xi->data_type == ELEM_FLOAT) {
            	load_categorical_node_hdfs<float>(result, fin);
            } else if(Xi->data_type == ELEM_DOUBLE) {
            	load_categorical_node_hdfs<double>(result, fin);
            } else if(Xi->data_type == ELEM_CHAR) {
            	load_categorical_node_hdfs<char>(result, fin);
            } else if(Xi->data_type == ELEM_STRING) {
            	load_categorical_node_hdfs<string>(result, fin);
            } else {
                cout<< "File = " << __FILE__ << ", Line = " << __LINE__ << "[ERROR] Unknown attribute(Xi) of type " << Xi->data_type << endl;
                exit(-1);
            }
        ...
        fin >> result->class_vector;
        //recurse on subtrees
        result->left = load_subtree_hdfs(dataset, y_index, fin);
        result->right = load_subtree_hdfs(dataset, y_index, fin);
    }

    return result;
}

vector<TreeNode*> load_forest_hdfs(Matrix & dataset, int y_index, const char* path)
{
    vector<TreeNode*> forest;
    HdfsReaderStream fin(path);
    ...
    for(size_t index = 0; index < n_tree; index++) {
        forest.push_back(load_subtree_hdfs(dataset, y_index, fin));
    }
