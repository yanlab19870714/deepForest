...
public:
	...
	//program entry point
	void run_MGS(const WorkerParams &params, int win_side, int col_factor, const char* hdfs_tree_dir) {
...
#pragma omp parallel for num_threads(n_threads)
	...
	*/
	...
	void run_CF0(const WorkerParams &params, int num_cols, int col_factor, const char* hdfs_tree_dir) {
		assert(params.replicate < _num_workers); //otherwise, a machine will keep a column more than once
		//note that only n-1 slaves are partitioning the data
...
#pragma omp parallel for num_threads(n_threads)
	...
	void run_CFi(const WorkerParams &params, int num_cols1, int col_factor1, const char* train_dir2, int num_cols2, int col_factor2, const char* hdfs_tree_dir) {
