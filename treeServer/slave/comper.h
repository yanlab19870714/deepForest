...
void set_stop_cond_and_indices(vector<size_t>::iterator start, vector<size_t>::iterator end, TreeConfig & treeConfig,
    ...
    vector<double> & class_vector = best_rows.class_vector;
    ...
    if(Y->data_type == ELEM_BOOL) { // only categorical
        ...
        if(!Y->is_ordinal) {
            stop = stop_splitting_categorical<bool>(cserver.X, start, end, treeConfig, label, class_vector);
        } else {
        ...
        if(Y->is_ordinal) {
            ...
            stop = stop_splitting_categorical<short>(cserver.X, start, end, treeConfig, label, class_vector);
        }
        ...
        if(Y->is_ordinal) {
            ...
            stop = stop_splitting_categorical<int>(cserver.X, start, end, treeConfig, label, class_vector);
        }
        ...
        if(Y->is_ordinal) {
            ...
            stop = stop_splitting_categorical<float>(cserver.X, start, end, treeConfig, label, class_vector);
        }
        ...
        if(Y->is_ordinal) {
            ...
            stop = stop_splitting_categorical<double>(cserver.X, start, end, treeConfig, label, class_vector);
        }
        ...
        if(!Y->is_ordinal) {
            stop = stop_splitting_categorical<char>(cserver.X, start, end, treeConfig, label, class_vector);
        } else {
            cout << "Error: Wrong type, File = " << __FILE__ << ", Line = " << __LINE__ << endl;
            exit(-1);
        }
        ...
        if(!Y->is_ordinal) {
            stop = stop_splitting_categorical<string>(cserver.X, start, end, treeConfig, label, class_vector);
        } else {
            cout << "Error: Wrong type, File = " << __FILE__ << ", Line = " << __LINE__ << endl;
            exit(-1);
        }
...
void create_end_split_response(Task_Slave_Col_Split* task, conque_p<resp2master> & ready_buffer) {
    ...
    if(y_data_type == ELEM_BOOL) {
        ...
        resp->class_vector = task->candidate_rows.class_vector;
...
public:
    ...
    void build_subtree(Task_Slave* task_slave) {
        ...
        if(subtree_task->candidate_rows.stop_splitting) {
            ...
            subtree_task->root = create_leaf_wrapper(Y, rows.begin(), rows.end(), subtree_task->candidate_rows.freq_y,
                                                                  subtree_task->candidate_rows.class_vector);
    ...
    void set_best_split(vector<size_t>::iterator start, vector<size_t>::iterator end, Column* column,
        ...
        vector<double> & class_vector = task_slave->candidate_rows.class_vector;
        ...
        c_resp->class_vector = class_vector;
    ...
    void stop_splitting(vector<size_t>::iterator start, vector<size_t>::iterator end,
        ...
        vector<double> & class_vector = task->candidate_rows.class_vector;
