...
//group_id -> machine_list mapping
vector<vector<int>> mac_map; //mac_map[group_id] = list of machine IDs
//need to resize() after loading metadata !!!

vector<data_source *> group_src; //group_src[group_id] = data_source of the group
vector<int> group_in_src; //group_in_src[group_id] = group_id in the data_source of the group

vector<int> group_size; //group_size[group_id] = number of columns in the group

// column_id -> group_id map
// column_ids (0......._num_columns - 1)
// group_ids (0.......GROUP_OF_COLUMNS - 1)

vector<int> col_group; //col_group[col_id] = group_id;

//_num_columns should have been set by metafiles
void init_column_group(vector<data_source> & sources, data_source & ysrc) { //sets GROUP_OF_COLUMNS, later used to resize mac_map
	//sets group_src, group_in_src, group_size, col_group, y_index

	//====== get the number of column groups
	GROUP_OF_COLUMNS = 0;
	for(int i=0; i<sources.size(); i++) GROUP_OF_COLUMNS += sources[i].n_group; //not considering y
	GROUP_OF_COLUMNS ++; //counting y
	//======
	group_src.resize(GROUP_OF_COLUMNS);
	group_size.resize(GROUP_OF_COLUMNS);
	group_in_src.resize(GROUP_OF_COLUMNS);
	col_group.resize(_num_columns);
	...
	{
		data_source & dsrc = sources[i];
		for(int j = 0; j < dsrc.n_group; j++)
		{
#ifdef ASSERT
			assert(group_idx<GROUP_OF_COLUMNS);
#endif
			group_src[group_idx] = &dsrc;
			group_in_src[group_idx] = j;
			group_size[group_idx] = dsrc.cols_per_group;
			if(j == dsrc.n_group - 1)
			{
				group_size[group_idx] = dsrc.n_col - (dsrc.n_group - 1) * dsrc.cols_per_group;
			}
			for(int k = 0; k < group_size[group_idx]; k++)
			{
#ifdef ASSERT
				assert(column_idx<_num_columns);
#endif
				col_group[column_idx] = group_idx;
				column_idx++;
			}
			//------
			group_idx++;
...
void set_col_split_assignment(size_t D, vector<vector<int>> & mac2colList,
    ...
    for(size_t i = 0; i < cols.size(); i++) {
        vector<int> & machine_indices = mac_map[col_group[cols[i]]];
...
void set_subtree_assignment(subtree_plan* s_plan, vector<Load_Content> & task_load) {
    ...
    for(size_t i = 0; i < cols.size(); i++) {
        vector<int> & mac_indices  = mac_map[col_group[cols[i]]];
...
public:
    ...
    void task_update_col_split_with_leaf(obinstream & m) {
        ...
        if(task_split->n_met == task_split->slave_ids.size()) {
            ...
            leafNode->class_vector.swap(resp.class_vector);
            //### hook it to the tree
    ...
    void create_node_and_plans(Task_Master_Col_Split* task_split) {
        ...
        if(task_best_split->isOrdinal) {
            ...
            treeNode->class_vector.swap(task_split->class_vector);
    ...
    void task_update_col_split(obinstream & m, int best_column_index) {
        ...
        //now existing task object is not leaf-responded
        ...
        if ((task_split->best_impurity < resp.best_split->best_impurity) ||
            ...
            task_split->class_vector.swap(resp.class_vector);
    ...
    void thread_func(char * buf, int size) {
        ...
        while (m.end() == false) { // get each msg in this batch
            ...
            if(resp_type == SUB_TREE_RESP) { // subtree response
                ...
                leaf_task->class_vector.swap(resp.class_vector);
