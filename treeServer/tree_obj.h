...
// output: predicted_y
template <class T>
void predict(TreeNode* root, size_t row_idx, T &predicted_y, vector<double> & class_vector,
		Matrix & dataset, int MAX_TEST_DEPTH) {
    TreeNode* temp = root;
        ...
        if(column->is_ordinal) {
            if(column->data_type == ELEM_SHORT) {
                ...
                if(depth == MAX_TEST_DEPTH) {
                    ...
                    class_vector = node->class_vector;
                    return;
                }
    ...
    class_vector = leafNode->class_vector;
    return;
}


// input: Tree_Root, dataset, y
// output: predicted_y values, prob_vecs : one vec for each data row
template <class T>
void predict(TreeNode* root, vector<T> &predicted_ys, vector<vector<double>> &prob_vecs, Matrix & dataset, int MAX_TEST_DEPTH) {
    int n_sample = dataset.get_column(0)->size;
    prob_vecs.resize(n_sample);

    for(size_t i = 0; i < n_sample; i++) {
        T predicted_y;
        predict<T>(root, i, predicted_y, prob_vecs[i], dataset, MAX_TEST_DEPTH);
        predicted_ys.push_back(predicted_y);
    }
}

template <class T>
void predict_forest(vector<TreeNode*> rootList, size_t row_idx, vector<double> & output_vector, Matrix & dataset, int MAX_TEST_DEPTH) {
	output_vector.resize(y_classes.size(), 0.0);
	T predicted_y; //dummy

	size_t n_trees = rootList.size();

	for(size_t i=0; i<n_trees; i++)
	{
		TreeNode* root = rootList[i];
		vector<double> class_vector;
		predict(root, row_idx, predicted_y, class_vector, dataset, MAX_TEST_DEPTH);
		for(int j=0; j<y_classes.size(); j++)
		{
			output_vector[j] += class_vector[j];
		}
	}

	for(size_t j=0; j<y_classes.size(); j++)
	{
		output_vector[j] /= n_trees;
	}
}
...
double get_accuracy(Matrix & dataset, vector<TreeNode*> & rootList, int y_index, int MAX_TEST_DEPTH) {
    ...
    for(size_t index = 0; index < D; index++) {
    	...
    	for(int i=0; i<predictions.size(); i++)
    	{
    		...
    		predict<T>(rootList[i], index, yhat, dummy_vector, dataset, MAX_TEST_DEPTH);
    		predictions[i] = yhat;
    	}
    	//------
