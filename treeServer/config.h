...
void print_job_config() {
    ...
    cout<< "Y classes : ";
	for(size_t index = 0; index < y_classes.size(); index++) {
		cout << y_classes[index];
		if(index != y_classes.size() - 1) {
			cout << ", ";
		}
	}
...
void load_job_config(const char* job_file) {
    ...
    line = reader.getLine();
	while(line.length() == 0 || line[0] == '#') line = reader.getLine(); // skip blank lines or lines starting with #
	istringstream save_tree_hdfs(line);
	getline(save_tree_hdfs, value, '#');
	SAVE_TREE_HDFS = atoi(value.c_str());
    ...
    //# get classes (deep forest)
    reader.readLine();
    line = reader.getLine();
	while(line.length() == 0 || line[0] == '#') line = reader.getLine(); // skip blank lines or lines starting with #
	istringstream class_line(line);
	getline(class_line, value, '#');

	istringstream sin(value);

	while (getline(sin, value, ',')) {
		boost::trim(value);
		ymap[value] = y_classes.size();
		y_classes.push_back(value);
	}
...
#ifdef DEBUG_LOG
            fout << "num trees found = " << t_config.num_trees << endl;
#endif
        }
...
#ifdef DEBUG_LOG
            fout << "MAX_TREE_DEPTH found = " << t_config.MAX_TREE_DEPTH << endl;
#endif
...
#ifdef DEBUG_LOG
            fout << "IMPURITY_FUNC found = " << t_config.IMPURITY_FUNC << endl;
#endif
...
#ifdef DEBUG_LOG
            fout << "MIN_SAMPLE_LEAF found = " << t_config.MIN_SAMPLE_LEAF << endl;
#endif
