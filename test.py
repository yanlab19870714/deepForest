...
LOCAL_TRAINING_DATA = 'mnist/test.csv'.format(HOME_DIRECTORY) # <-------------------------------------- here
#////////////////////////////////////////////////////////////

#////////////////////////////////////////////////////////////
# HDFS directories
HDFS_HOME_DIRECTORY = 'mnist_demo'
HDFS_TRAINING_DATA_DIRECTORY = '{}/data_test'.format(HDFS_HOME_DIRECTORY) # <-------------------------------------- here
HDFS_MGS_DIRECTORY = '{}/mgs_test'.format(HDFS_HOME_DIRECTORY) # phase 1: multi-grained scaling # <-------------------------------------- here
HDFS_TRAIN_MGS_DIRECTORY = '{}/mgs'.format(HDFS_HOME_DIRECTORY) # phase 1: multi-grained scaling # <-------------------------------------- here
#////////////////////////////////////////////////////////////
...
def put():
	...
	cmd = 'hadoop fs -rmr {}'.format(HDFS_TRAINING_DATA_DIRECTORY) # <-------------------------------------- here: don't remove root
	print(cmd)
	os.system(cmd)
	print("") # empty line
...
def prepare_win_job_config(win):
	...
	content += '0 # SAVE_TREE_HDFS: 1 = true, 0 = false\n' # <-------------------------------------- here: don't create tree
...
def mgs_train_win(win): # <-------------------------------------- here: should not be called
...
def mgs_train_wins(): # <-------------------------------------- here: should not be called
	for win in window_list:
		mgs_train_win(win)
...
def mgs_convert_win(win):
	...
	output_hdfs_dir = HDFS_HOME_DIRECTORY + '/' + str(win) + 'test' # <-------------------------------------- here
	model_dir = HDFS_TRAIN_MGS_DIRECTORY + '/' + str(win) + '/models' # <-------------------------------------- here
...
def prepare_job_config_CF0(win):
	...
	content += ' # subtree_D, threshold to decide on subtree task or col_split task\n'
	content += '1 # NO_WARNING: 1 = true, 0 = false\n'
	content += HDFS_HOME_DIRECTORY + '/' + str(win) + 'test' # <-------------------------------------- here
	content += ' # HDFS path train file\n'
	content += 'NOT_USED # HDFS path meta file, not used in deep forest as it will be reset as train_file/meta.csv # not used in testing\n'
	...
	hdfs_file_path = HDFS_HOME_DIRECTORY + '/job_CF0test.config' # <-------------------------------------- here
...
def cf0_train(win): # <-------------------------------------- here: should not be called
	feature_num = (WIDTH - win + 1) * (HEIGHT - win + 1) * len(y_classes) * (mgs_forests + mgs_extratrees)
...
def cf0_convert(win):
	...
	hdfs_data_dir = HDFS_HOME_DIRECTORY + '/' + str(win) + 'test' # <-------------------------------------- here
	output_hdfs_dir = HDFS_HOME_DIRECTORY + '/CF0test' # <-------------------------------------- here
	...
	cmd += ' '
	cmd += HDFS_HOME_DIRECTORY + '/job_CF0test.config' # <-------------------------------------- here
	cmd += ' '
	cmd += HDFS_HOME_DIRECTORY + '/CF0_models' # <-------------------------------------- here: reuse trained models
	cmd += ' '
	cmd += str(num_compers)
	print(cmd)
	os.system(cmd)
...
def cf_train(prev_win, round): # <-------------------------------------- here: should not be called
...
def cf_convert(prev_win, round):
	...
	job_config_file = HDFS_HOME_DIRECTORY + '/job_CF' + str(round-1) + 'test.config' # <-------------------------------------- here
	model_dir = HDFS_HOME_DIRECTORY + '/CF' + str(round) + '_models'
	data2dir = HDFS_HOME_DIRECTORY + '/CF' + str(round-1) + 'test' # <-------------------------------------- here

	data1dir = HDFS_HOME_DIRECTORY + '/' + str(prev_win) + 'test' # <-------------------------------------- here
	output_hdfs_dir = HDFS_HOME_DIRECTORY + '/CF' + str(round) + 'test' # <-------------------------------------- here
...
def prepare_job_config_CF(win, round):
	...
	hdfs_file_path = HDFS_HOME_DIRECTORY + '/job_CF' + str(round) + 'test.config' # <-------------------------------------- here
...
while round<=ROUND:
	...
	win = window_list[pos]
	print("#################### CF{} Feature Extraction with Win {} ####################".format(round, win))
